% Render: python3 /Users/junix/projects/skills/create-tikz-infographic/scripts/render_tikz.py docs/architecture-infographic.tex --workdir /Users/junix/projects/plot/plot-ts --out-dir /tmp/tikz-plot-ts --engine xelatex --dpi 300 --svg --strict-warnings --strict-geometry --expect-text "plot-ts"
% Claim: 调用者经入口进入核心与支撑模块，最终得到可观察结果。
% Reader question: plot-ts 的系统边界、主路径与验证边界是什么？
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[anchor=west,font=\footnotesize\bfseries,text=Ocean] at (0,8.88) {项目架构信息图 · typescript};
  \node[anchor=west,text width=15.8cm,align=left,font=\Large\bfseries,text=Ink] at (0,8.25) {plot-ts};
  \node[anchor=west,font=\small,text=Slate] at (0,7.42) {图形与可视化：入口、模块、依赖与验证边界};
  \node[external] (caller) at (1.55,5.15) {调用者\\{\footnotesize 人或上游系统}};
  \node[component] (interface) at (5.05,5.15) {库接口\\{\footnotesize 公开包接口}};
  \node[component,line width=1.2pt] (core) at (8.55,5.15) {svg\\{\footnotesize 核心职责}};
  \node[component] (output) at (14.45,5.15) {调用结果};
  \node[component] (support) at (6.75,2.75) {style\\{\footnotesize 支撑职责}};
  \node[external] (dependency) at (10.35,2.75) {@types/d3\\{\footnotesize 包清单边界}};
  ...
  \end{pgfonlayer}
  \node[note,anchor=west,text width=15.8cm,align=left] at (0,.72) {主路径：调用者 $\rightarrow$ 库接口 $\rightarrow$ svg $\rightarrow$ 调用结果。点线表示由入口与源码布局推得的关系，不表达性能或调用次数。};
  \draw[flow] (0,.22) -- (.9,.22);\node[note,anchor=west] at (1.05,.22) {确定边界};
  \draw[inferred] (3.25,.22) -- (4.15,.22);\node[note,anchor=west] at (4.3,.22) {推断关系};
  \node[note,anchor=east] at (16,.22) {代码快照 910a5af};
\end{tikzpicture}
\end{document}
